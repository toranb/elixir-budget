defmodule Example.TransactionsTest do
  ...
  @transaction_two %{description: "two", amount: 200, user_id: @id_two, category_id: @category_id_two}
  @transaction_three %{description: "three", amount: 300, user_id: @id, category_id: @category_id_one}
  ...
  test "all will return only transactions for the given user_id" do
    ...
    %Transaction{:description => description, category_id: category_id} = Enum.find(transactions, fn(t) -> t.amount == 100 end)
    assert description == "one"
    assert category_id == @category_id_one
    %Transaction{:description => description, category_id: category_id} = Enum.find(transactions, fn(t) -> t.amount == 300 end)
    assert description == "three"
    assert category_id == @category_id_one

    transactions = Transactions.all(@user_two.id)
    assert Enum.count(transactions) == 1
    %Transaction{:description => description, category_id: category_id} = Enum.find(transactions, fn(t) -> t.amount == 200 end)
